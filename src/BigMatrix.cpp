...
template<typename T>
void* ResizeSharedMatrix(const std::string &sharedName,
  MappedRegionPtrs &dataRegionPtrs, const index_type numRow,
  const index_type numCol, const bool readOnly=false)
  {
    using namespace boost::interprocess;

  try 
  {
    // open the shared memory object (shm)
    shared_memory_object shm(open_only, sharedName.c_str(), 
      read_write);
    // resize the shm
    shm.truncate( numRow*numCol*sizeof(T) );
            
//    dataRegionPtrs.resize(numRow);
//    for(unsigned i=0; i < numRow; i++){
//        dataRegionPtrs[i].resize(numCol);
//    }
    
    // This is probably a major point of interest
    dataRegionPtrs.push_back(
      MappedRegionPtr( new MappedRegion(shm, read_write)));
...
bool SharedMemoryBigMatrix::resize( const std::string &uuid, 
  const index_type numRow, const index_type numCol, const int matrixType, 
  const bool sepCols, const bool readOnly )
  {
    using namespace boost::interprocess;
    ...
    else
    {
      switch(_matType)
      {
        case 1:
          try
          {
            _pdata = ResizeSharedMatrix<char>(_sharedName, _dataRegionPtrs, 
              _nrow, _ncol, _readOnly);
          }
          catch(boost::interprocess::interprocess_exception &e)
          {
            if (!_readOnly)
        ...
        case 2:
          try
          {
            _pdata = ResizeSharedMatrix<short>(_sharedName, _dataRegionPtrs, 
              _nrow, _ncol, _readOnly);
          }
          catch(boost::interprocess::interprocess_exception &e)
          {
        ...
        case 4:
          try
          {
            _pdata = ResizeSharedMatrix<int>(_sharedName, _dataRegionPtrs, 
             _nrow, _ncol, _readOnly);
          }
          catch(boost::interprocess::interprocess_exception &e)
          {
        ...
        case 8:
          try
          {
            _pdata = ResizeSharedMatrix<double>(_sharedName, _dataRegionPtrs, 
              _nrow, _ncol, _readOnly);
          }
          catch(boost::interprocess::interprocess_exception &e)
          {
